...
import {
  doRequestToPlayMatch,
  doConfirmandDeclineMatch,
  doSentFriendRequest,
  doSetNotificationData,
} from "../../../../redux/actions/AppActions";
import { EventRegister } from "react-native-event-listeners";

class UserRequestScreen extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.responseRequestToPlayMatchdata !==
      this.props.responseRequestToPlayMatchdata
    ) {
      if (this.props.responseRequestToPlayMatchdata !== undefined) {
  ...
  onClickRequestToPlay = async () => {
    const { matchDetail } = this.state;
    if (matchDetail.is_status == "Requested") {
      showErrorMessage(errors.alreadyRequested);
    } else {
      if (globals.isInternetConnected == true) {
        const params = {
          match_id: matchDetail.id,
        };
        this.props.doRequestToPlayMatch(params);
      } else {
  ...
  render() {
    ...
    return (
      <View style={[TabCommonStyle.container]}>
        ...
        >
          {matchDetail.is_status == "Requested" ? (
            <CustomOnebuttonComponent
              segmentOneTitle={strings.cancelRequest}
              segmentOneImage={images.close_img}
              onPressSegmentOne={() => this.onClickCancelRequest()}
            />
          ) : (
            <CustomOnebuttonComponent
              segmentOneTitle={
                matchDetail.is_status == "Requested"
                  ? strings.alreadyReuested
                  : strings.requestToPlay
              }
              segmentOneImage={images.round_right_img}
              onPressSegmentOne={() => this.onClickRequestToPlay()}
            />
          )}
        </View>
        ...
        />
        {this.props.isBusyRequestToPlayMatch ||
        this.props.isConfirmAndDeclineMatch ||
        this.state.isBusySentFriendRequest ? (
          <Loader />
        ) : null}
      </View>
    );
...
function mapStateToProps(state) {
  return {
    ...
    isBusyRequestToPlayMatch: state.app.isBusyRequestToPlayMatch,
    responseRequestToPlayMatchdata: state.app.responseRequestToPlayMatchdata,
...
export default connect(mapStateToProps, mapDispatchToProps)(UserRequestScreen);
